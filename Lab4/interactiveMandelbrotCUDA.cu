...
__device__ int mandelbrot( int x, int y, int maxiter, int gImageWidth, int gImageHeight, int offsetx, int offsety, int scale)
{
...
__global__ void computeFractal( unsigned char *ptr, int maxiter, int gImageWidth, int gImageHeight, int offsetx, int offsety, int scale)
{
    // map from x, y to pixel position
		...
		int idy = blockIdx.y * blockDim.y + threadIdx.y;
		    ...
		    int fractalValue = mandelbrot( idx, idy, maxiter, gImageWidth, gImageHeight, offsetx, offsety, scale);
				//printf("thread id = %d,%d  offset = %d with return value = %d\n", idx, idy, offset, fractalValue);
...
void Draw()
{
	...
	cudaEvent_t start;
		...
		cudaMalloc(&gpuPixels, DIM * DIM * 4);
		cudaMemcpy(gpuPixels, pixels, DIM * DIM * 4, cudaMemcpyHostToDevice);

		dim3 dimBlock( BLOCKSIZE, BLOCKSIZE );
		dim3 dimGrid( gImageHeight/BLOCKSIZE, gImageWidth/BLOCKSIZE );
		//printf("dimGrid = (%d, %d)", gImageHeight/BLOCKSIZE, gImageWidth/BLOCKSIZE);
		cudaEventRecord(start, 0);
		computeFractal<<<dimGrid, dimBlock>>>(gpuPixels, maxiter, gImageWidth, gImageHeight, offsetx, offsety, scale);
		...
		cudaMemcpy(pixels, gpuPixels, DIM * DIM * 4, cudaMemcpyDeviceToHost);
		/*
		for (size_t i = 0; i < DIM; i+=10) {
			for (size_t j = 0; j < DIM; j+=10) {
				printf("pixels value = %d, %d, %d, %d", pixels[(i*DIM + j) * 4], pixels[(i*DIM + j) * 4 + 1], pixels[(i*DIM + j) * 4 + 2], pixels[(i*DIM + j) * 4 + 3]);
			}
		}
		*/
